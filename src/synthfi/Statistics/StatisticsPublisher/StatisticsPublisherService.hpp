...
class StatisticsPublisherService : public boost::asio::execution_context::service
{
public:
    // Constructor creates a thread to run a private io_service.
    StatisticsPublisherService( boost::asio::execution_context & executionContext, const StatisticsPublisherConfig & config )
    ...
    StatisticsPublisherImpl & impl( ) { return _impl; }

    boost::asio::strand< boost::asio::io_context::executor_type > & strand( ) { return impl( ).strand( ); }
...
private:
    ...
    {
        SYNTHFI_LOG_DEBUG( _logger ) << "Shutting down StatisticsPublisherService";
    }
